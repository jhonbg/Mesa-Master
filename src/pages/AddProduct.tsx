...
const AddProduct: React.FC = () => {
  ...
  const handleFinalPrice = async (idProducto: number) => {
    try {
      const response = await axios.get(`http://localhost:8090/laempacadora/api/detalleproducto/pedido/${idPedido}`);
...
try {
    ...
    console.log(precio);
    let precioF = 0;
    if(precioFinal == 0) {precioF = (precio*parseInt(productQuantity))} else { precioF = precioFinal+(precio*parseInt(productQuantity))};
        ...
        <Paper elevation={24} style={{ padding: '50px', marginBottom: '50px', maxWidth: '500px', width: '100%', textAlign: 'center' }}>
            ...
            <Container
              ...
              style={{
                ...
                overflowY: 'auto', // Agrega el scroll vertical
              }}
            >
                <TextField
