...
const OrderHome: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
          ...
          const response = await axios.post('https://mesamaster-backend.onrender.com/api/pedidos/save', jsonUser);
  ...
  const handleMesa = async () => {
    try {
      if (idMesa !== 0) { 
        ...
        const response = await axios.put('https://mesamaster-backend.onrender.com/api/mesas/actualizar', jsonTable);
        setError('');
      }
    } catch (error) {
      console.log(error)
